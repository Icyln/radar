...
Phase 7E adds persistent discovery-run health and multi-source job observations. Phase 7D itself did not require a migration.
...
- deeper bounded Himalayas/Arbeitnow pagination
- provider-level success/failure isolation and warnings
- persistent `discovery_runs` automation history
- Overview health cards for direct monitoring and Wide Search
- cross-source `job_source_observations` and conservative job convergence
- WIDE Active → Unknown → Closed lifecycle retirement
- GitHub Source Discovery run identity recording
- UI-visible feed-page/source/dedup metrics

Migration head: `0010_phase7e`
