...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    ["Monitored companies", data.monitored_companies],
  ...
  return <><PageHeader eyebrow="Overview" title="Your job radar" description="A concise view of matching activity and monitoring health." action={<Link href="/profiles" className="button-primary">New profile</Link>} />
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{stats.map(([label,value]) => <div key={label} className="panel p-4"><p className="text-xs text-zinc-500">{label}</p><p className="mt-2 text-2xl font-semibold tracking-tight text-zinc-50">{value}</p></div>)}</section>
